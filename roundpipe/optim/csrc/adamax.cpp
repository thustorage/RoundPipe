...
#pragma omp parallel
    {
        ...
        for (size_t i = 0; i < params.size(); ++i) {
            ...
            float *exp_inf_ptr = exp_inf[i].mutable_data_ptr<float>() + offset;
            adamax_kernel(maximize, weight_decay == 0.0, params_ptr, grads_ptr,
                          exp_avg_ptr, exp_inf_ptr, beta1, beta2, eps, weight_decay,
                          clr, block_size);
        }
    }
}
