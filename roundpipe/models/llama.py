...
def wrap_model(model: LlamaForCausalLM, **roundpipe_kwargs: Any) -> RoundPipe:
    model.loss_function = CompileForCausalLMLoss
    prefix = LlamaForCausalLMPrefix(model)
    layers = [LlamaForCausalLMWrappedLayer(layer) for layer in model.model.layers]
    postfix = LlamaForCausalLMPostfix(model)
